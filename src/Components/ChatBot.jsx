...
export default function ChatBot() {
  const [messages, setMessages] = useState([
    ...
    { id: 2, text: 'Look for signs like discolored leaves, unusual growths on the trunk, wilting, or dead branches. Upload a photo, and I can help identify the issue.', sender: 'bot' },
  ]);

  const getBotResponse = (userMessage) => {
    return new Promise((resolve) => {
      setTimeout(() => {
        resolve({ text: 'This is a sample response from the bot.', sender: 'bot' });
      }, 1000);
    });
  };
  ...
  const handleUserMessage = async (text) => {
    ...
    setMessages((prevMessages) => [...prevMessages, { ...botResponse, id: Date.now() }]);
  };
